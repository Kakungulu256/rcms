...
async function setupTenants() {
  ...
  await ensureIndex(() =>
    databases.createIndex(databaseId, collectionId, "idx_house", "key", ["houseId"])
  );
  await ensureIndex(() =>
    databases.createIndex(databaseId, collectionId, "idx_status", "key", ["status"])
  );
...
async function setupPayments() {
  ...
  await ensureIndex(() =>
    databases.createIndex(databaseId, collectionId, "idx_tenant", "key", ["tenantId"])
  );
  await ensureIndex(() =>
...
async function setupExpenses() {
  ...
  await ensureAttribute(() =>
    databases.createStringAttribute(databaseId, collectionId, "houseId", 64, false)
  );
  await ensureAttribute(() =>
    databases.createStringAttribute(
      databaseId,
